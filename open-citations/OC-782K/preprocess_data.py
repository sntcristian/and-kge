...
with open(folder+'entity_to_id.json', 'w') as f1:
    json.dump(tf.entity_to_id, f1)
f1.close()


with open(folder+'relation_to_id.json', 'w') as f2:
...
training_df.to_csv(folder+"training.txt", sep="\t", header=False, index=False)
validation_df.to_csv(folder+"validation.txt", sep="\t", header=False, index=False)
testing_df.to_csv(folder+"testing.txt", sep="\t", header=False, index=False)